...
$stmt->execute([$id]);
$row = $stmt->fetch();
if (!$row) { setFlash('error', 'Data tidak ditemukan.'); redirect(APP_URL . '/modules/suppliers/index.php'); }
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '') {
        ...
        $stmt = $pdo->prepare("UPDATE suppliers SET name=?, contact_person=?, phone=?, address=? WHERE id=?");
...
<div class="card" style="max-width:600px;">
    <div class="card-body">
        ...
        <form method="POST">
            <?= csrfField() ?>
            <div class="mb-3"><label class="form-label">Nama Supplier</label><input type="text" name="name" class="form-control" value="<?= e($_POST['name']) ?>" required></div>
